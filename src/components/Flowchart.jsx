...
export default function FlowChart() {
    ...
    const [vw, setVw] = useState(1920); // 실제 뷰포트 폭(최대 1920)
    ...
    useEffect(() => {
        const update = () => {
            ...
            if (!el) return;
            const w = Math.min(1920, Math.round(el.clientWidth || 0));
            if (w > 0) setVw(w);
        };
        update();
    ...
    const segments = useMemo(() => {
        if (!nat1.w) return 0;
        return Math.ceil(nat1.w / 1920);
    }, [nat1.w]);

    // 스케일 = 실제 뷰포트 폭 / 1920
    const scale = useMemo(() => (vw ? vw / 1920 : 1), [vw]);

    // Swiper 로드 & 초기화 (Pagination)
    useEffect(() => {
            ...
            <div className="panel-body">
                <div className="panel-content">
                    ...
                    <InfoSection
                        ...
                        note="이 Flow Chart는 큰 UX 경로 단위의 탐색 흐름을 표현한 것입니다. 각 플로우(Create, Stage, Magazine, My Page 등)는 독립적으로 나뉘어 있으나, 실제 경험에서는 각 플로우가 상호보완적으로 연결되어 하나의 통합 서비스 경험을 제공합니다."
                    />
                    ...
                    {/* 공통 래퍼: 정렬/폭 공통 적용 */}
                    <div ref={wrapRef} className={`fc-wrap ${alignClass}`}>
                        {/* flowchart_1: 1920 단위 슬라이드(유실 없이 이어짐) */}
                        <div className="swiper mySwiper fc-swiper">
                                ...
                                {Array.from({ length: segments || 1 }).map((_, idx) => (
                                    <div className="swiper-slide" key={idx}>
                                        <div
                                            ...
                                            style={{ width: `${vw}px`, height: `${displayH1}px` }}
                                        >
                                            <img
                                                src={IMG1}
                            ...
                            {/* 점 페이지네이션: 아래로 내리기 위해 클래스 추가 */}
                            <div className="swiper-pagination fc-pagination" />
                        </div>

                        {/* 슬라이더와 2번 이미지 사이 여백 */}
                        <div className="fc-spacer-md" />

                        {/* flowchart_2: 같은 스케일/정렬로 표시 */}
